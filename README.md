 ...
 * Transport: Peer connection used to send and receive data
 * Inbound: Measurements of the network traffics incoming to the monitored units
 * Outbound: Measurements of the network traffics outgoing from the monitored units
 * Extension: Custom added events and objects (frozen video, high latency, etc...)

Each report generated by the observer is based on a decomposed part extended with results of analysis.

For example the report type InboundAudioTrackReport is based on the inbound part of the source sample. Inbound part is
extended with the following information: 1) identifier of the call it belongs to; 
2) identifier of the client the track is originated from;

### Collect Samples

The observer collects Samples on the following endpoints:
...
You can specify the encoding format of the Samples by specifying `?format={FORMAT}` when the endpoint is requested to open a connection.
The format can be `json`, or `protobuf`, by default it is `json`.
