...
class ExpiredPCsEvaluatorTest {
    ...
    public void shouldRemovePeerConnection() {
        ...
        Assertions.assertEquals(1, reports.stream().filter(r -> r.value.getType().equals(ReportType.DETACHED_PEER_CONNECTION)).count());
        Assertions.assertEquals(0, reports.stream().filter(r -> r.value.getType().equals(ReportType.FINISHED_CALL)).count());
    ...
    public void shouldRemoveCall() {
        ...
        Assertions.assertEquals(1, reports.stream().filter(r -> r.value.getType().equals(ReportType.DETACHED_PEER_CONNECTION)).count());
        Assertions.assertEquals(1, reports.stream().filter(r -> r.value.getType().equals(ReportType.FINISHED_CALL)).count());
