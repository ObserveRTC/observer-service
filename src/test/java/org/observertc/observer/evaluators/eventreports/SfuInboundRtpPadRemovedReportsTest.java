...
@MicronautTest
class SfuInboundRtpPadRemovedReportsTest {
    ...
    void shouldHasExpectedValuesWhenRemoved() throws Throwable {
        var sfuRtpPadDTO = this.modelsGenerator.getSfuRtpPadDTO();

        var reports = this.sfuInboundRtpPadRemovedReports.mapRemovedSfuRtpPad(List.of(sfuRtpPadDTO));
    ...
    void shouldHasExpectedValuesWhenExpired() throws Throwable {
        ...
        var reports = this.sfuInboundRtpPadRemovedReports.mapExpiredSfuRtpPad(List.of(expiredEvent));
        var actual = reports.get(0);
        var streamId = UUIDAdapter.toStringOrNull(sfuRtpPadDTO.streamId);
        var sinkId = UUIDAdapter.toStringOrNull(sfuRtpPadDTO.sinkId);
