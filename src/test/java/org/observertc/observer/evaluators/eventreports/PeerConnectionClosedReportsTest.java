...
class PeerConnectionClosedReportsTest {
    ...
    void shouldHasExpectedValuesWhenRemoved() throws Throwable {
        var expected = modelsGenerator.getPeerConnectionModel();

        var promise = new CompletableFuture<List<CallEventReport>>();
        this.peerConnectionClosedReports.getOutput().subscribe(promise::complete);
        this.peerConnectionClosedReports.accept(List.of(expected));
