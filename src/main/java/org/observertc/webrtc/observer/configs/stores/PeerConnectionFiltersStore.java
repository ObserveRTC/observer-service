...
@Singleton
public class PeerConnectionFiltersStore extends StoreAbstract<String, PeerConnectionFilterConfig> {
	private static final Logger logger = LoggerFactory.getLogger(PeerConnectionFiltersStore.class);
	...
	void setup() {
		...
		this.configDispatcher.onPCFiltersChanged()
				.map(event -> event.config)
				.subscribe(this::process);
	}

	private void process(ObserverConfig observerConfig) {
		Map<String, PeerConnectionFilterConfig> pcFilters = observerConfig.pcFilters
				.stream()
				.collect(Collectors.toMap(
						c -> c.name,
						java.util.function.Function.identity())
				);
		this.setMap(pcFilters);
	}
