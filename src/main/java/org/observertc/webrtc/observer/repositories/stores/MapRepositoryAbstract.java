...
public class MapRepositoryAbstract<K, V> implements Repository<K, V> {
	...
	@Override
	public <R extends K, U extends V> Optional<V> saveIfAbsent(@NotNull R key, @NotNull U value) {
		V result = this.entities.putIfAbsent(key, value);
	...
	public Map<K, V> getAllEntries() {
		Set<Map.Entry<K, V>> values = this.entities.entrySet();
		return values.stream().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	}

	@Override
	public Set<K> getTotalKeySet() {
		Set<K> keys = this.entities.keySet();
