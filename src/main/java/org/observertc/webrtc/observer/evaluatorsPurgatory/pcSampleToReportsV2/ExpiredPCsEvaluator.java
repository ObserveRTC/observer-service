...
import static org.observertc.webrtc.observer.evaluatorsPurgatory.pcSampleToReportsV2.Pipeline.REPORT_VERSION_NUMBER;
...
public class ExpiredPCsEvaluator implements Consumer<Map<UUID, PCState>> {
	...
	private PeerConnectionEntity detachPeerConnection(@NotNull PCState pcState) {
		...
		try {
			...
			this.serviceMetrics.incrementDetachedPCs(pcEntity.peerConnection.serviceName, pcEntity.peerConnection.mediaUnitId);
		} finally {
			return pcEntity;
		}
	}
	...
	private void finnishCall(UUID callUUID, Long timestamp) {
		...
		Report report = Report.newBuilder()
				...
				.build();
		this.reports.onNext(report);
		this.serviceMetrics.incrementFinishedCall(callEntity.call.serviceName);
		Long durationInMillis = timestamp - callEntity.call.initiated;
		Duration callDuration = Duration.ofMillis(durationInMillis);
		this.serviceMetrics.addCallDuration(callEntity.call.serviceName, callDuration);
	}
}
