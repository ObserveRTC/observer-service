...
public class ActivePCsEvaluator implements Consumer<Map<UUID, PCState>> {
	...
	private PeerConnectionEntity addNewPeerConnection(UUID callUUID, PCState pcState) {
		...
		try {
			...
			this.serviceMetrics.incrementJoinedPCs(pcEntity.peerConnection.serviceName, pcEntity.peerConnection.mediaUnitId);
		} finally {
			return pcEntity;
		}
	}
	...
	private CallEntity addNewCall(PCState pcState) {
		...
		try {
			...
			this.serviceMetrics.incrementInitiatedCall(callEntity.call.serviceName);
		} finally {
			return callEntity;
		}
	}
