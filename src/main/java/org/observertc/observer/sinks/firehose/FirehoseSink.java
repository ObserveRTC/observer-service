...
public class FirehoseSink extends Sink {
    ...
    protected void process(List<Report> reports) {
        ...
        for (var report : reports) {
            byte[] bytes = jsonMapper.map(report);
            if (bytes == null) {
        ...
        for (; retried < 3; ++retried) {
            ...
            try {
                ...
                logger.info("{} records are processed to stream {}", recordResponse.requestResponses().size(), this.streamName);
