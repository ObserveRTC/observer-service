...
@Prototype
public class OutboundVideoTrackRemovedReports {

    private static final Logger logger = LoggerFactory.getLogger(OutboundVideoTrackRemovedReports.class);
    ...
    public List<CallEventReport> mapRemovedOutboundVideoTrack(List<Models.OutboundVideoTrack> peerConnectionModels) {
        if (Objects.isNull(peerConnectionModels) || peerConnectionModels.size() < 1) {
            return Collections.EMPTY_LIST;
        }
        var peerConnectionIds = peerConnectionModels.stream().map(Models.OutboundVideoTrack::getPeerConnectionId)
    ...
    public List<CallEventReport> mapExpiredOutboundVideoTrack(List<RepositoryExpiredEvent<Models.OutboundVideoTrack>> expiredPeerConnectionDTOs) {
    ...
    private CallEventReport perform(PeerConnection peerConnection, Models.OutboundVideoTrack outboundVideoTrack, Long timestamp) {
        if (peerConnection == null) {
            logger.warn("Did not found peerConnection for outbound video track {}", outboundVideoTrack.getTrackId());
            return null;
        }
        var removed = peerConnection.removeOutboundVideoTrack(outboundVideoTrack.getTrackId());
        if (!removed) {
            logger.warn("Did not removed outbound video track {} from peer connection {}. Already removed?", outboundVideoTrack.getTrackId(), outboundVideoTrack.getPeerConnectionId());
            return null;
        }
        return this.makeReport(outboundVideoTrack, timestamp);
    }

    private CallEventReport makeReport(Models.OutboundVideoTrack mediaTrackDTO, Long timestamp) {
        try {
            ...
            MediaTrackAttachment attachment = MediaTrackAttachment.builder()
                    ...
                    .build();
            String message = String.format("Media Track is removed. sfuStreamId: %s, direction: %s", mediaTrackDTO.getSfuStreamId(), streamDirection);
