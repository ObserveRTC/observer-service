...
@Prototype
public class InboundVideoTrackAddedReports {

    private static final Logger logger = LoggerFactory.getLogger(InboundVideoTrackAddedReports.class);
    ...
    public List<CallEventReport> mapAddedInboundVideoTrack(List<Models.InboundVideoTrack> inboundVideoTracks) {
        if (Objects.isNull(inboundVideoTracks) || inboundVideoTracks.size() < 1) {
            return Collections.EMPTY_LIST;
        }

        var reports = inboundVideoTracks.stream()
                .map(this::makeReport)
    ...
    private CallEventReport makeReport(Models.InboundVideoTrack inboundAudioTrack) {
        try {
