...
public class TaskStage {
    ...
    public AtomicReference execute(AtomicReference inputHolder) throws Throwable{
        if (this.rolledBack || this.executed) {
            logger.info("Stage {} is already executed ({}) or rolled back ({}) execution forward is not allowed", this.executed, this.rolledBack);
            return null;
        }
    ...
    public void rollback(Throwable throwable)  {
        if (this.rolledBack || !this.executed) {
            logger.info("Stage {} is either not executed ({}) or already rolled back ({})", this.executed, this.rolledBack);
            return;
        }
