...
public class InitiatedCalls {
	private static final Logger logger = LoggerFactory.getLogger(InitiatedCalls.class);

	private static final String TABLE_NAME = "InitiatedCalls";
	private final WebRTCDataSet webRTCDataSet;
	private final BigQueryService bigQueryService;

	public InitiatedCalls(WebRTCDataSet webRTCDataSet, BigQueryService bigQueryService) {
		this.webRTCDataSet = webRTCDataSet;
		this.bigQueryService = bigQueryService;
	}

	public void insert(InitiatedCall initiatedCall) {
		TableId tableId = this.getTableId();

		InsertAllResponse response =
				this.bigQueryService.getBigQuery().insertAll(
						InsertAllRequest.newBuilder(tableId)
								.addRow(initiatedCall.toMap())
								.build());
