...
import static com.observertc.gatekeeper.jooq.Tables.OBSERVERORGANISATIONS;
import static com.observertc.gatekeeper.jooq.Tables.OBSERVERS;
import static com.observertc.gatekeeper.jooq.Tables.ORGANISATIONS;
...
public class ObserverRepository implements CrudRepository<ObserverDTO, UUID> {
	...
	public Iterable<EvaluatorDTO> findEvaluators(@NonNull @NotNull UUID uuid) {
		return this.contextProvider.get()
				.select(ORGANISATIONS.UUID, ORGANISATIONS.NAME, ORGANISATIONS.DESCRIPTION)
				.from(ORGANISATIONS
						.join(OBSERVERORGANISATIONS).on(OBSERVERORGANISATIONS.ORGANISATION_ID.eq(ORGANISATIONS.ID))
						.join(OBSERVERS).on(OBSERVERORGANISATIONS.OBSERVER_ID.eq(OBSERVERS.ID))
						.where(OBSERVERS.UUID.eq(UUIDAdapter.toBytes(uuid))))
